...
<template>
  <view class="bind-car">
    <template v-if="!addFlag">
      <view class="mt-28rpx w-711rpx rounded-10rpx overflow-hidden">
        <view class="mt-8rpx">
          <wd-cell-group border>
            <wd-input v-model="name" label-width="30%" type="text" label="车辆名字" placeholder="请输入车辆名称" />
            <wd-input v-model="brand" label-width="30%" type="text" label="姓名" placeholder="请输入姓名"  />
            <wd-input v-model="phone" label-width="30%" type="text" label="手机号" placeholder="请输入手机号"  />
            <wd-cell value="与车主关系">
              <template #title>
              ...
              <wd-picker v-model="color" :columns="columns" use-default-slot>
                <view class="flex items-center justify-end">
                  ...
                  <view v-else class="mr-15rpx color-[#C8C8C8] text-24rpx">
                    请选择
                  </view>
    ...
    <template v-else>
      <image
      class="w-348rpx h-288rpx mt-230rpx"
        :src="SuccessDefault"
        mode="scaleToFill"
      />
      <view class="flex items-center justify-between mt-261rpx">
        <view class="complete-btn" @click="onCompleteClick">完成</view>
        <view class="relative">
          <view class="w-260rpx h-92rpx absolute right-0 top-[-112rpx]" >
            <image
              class="absolute w-100% h-100% absolute right-0 top-0"
              :src="YellowTips"
              mode="scaleToFill"
            />
            <view class="relative text-[#6E6E6E] text-24rpx mt-10rpx  px-38rpx">去提醒朋友查收 一下吧！</view>
          </view>
          <view class="notice-btn" @click="onNoticeClick">通知成员</view>
        </view>
      </view>
    </template>
  </view>

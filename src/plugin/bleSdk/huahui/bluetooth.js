...
import dataTransition from './dataTransition.js'
import util from './util.js'

// const dataTransition = import('./dataTransition.js')
// const util = import('../util.js')
...
function BluetoothOperate() {
  ...
  const _startBluetoothDevicesDiscovery = () => {
    wx.startBluetoothDevicesDiscovery({
      ...
      fail: (err) => {
        if (err.errMsg.includes('not init')) {
          _initBluetooth(() => {
            _startBluetoothDevicesDiscovery()
          })
        }
    ...
    wx.onBluetoothDeviceFound((res) => {
      ...
      if (device && device.advertisData && device.advertisData.byteLength != 0) {
        ...
        if (machineNO == _machineNO) {
          ...
          if (_operateType == 'open' || _operateType == 'close' || _operateType == 'temp' || _operateType == 'tempopen' || _operateType == 'status' || _operateType == 'helmet')
    ...
    if (_deviceId.length > 0) {
      wx.createBLEConnection({
        ...
        fail: (err) => {
          ...
          else if (err.errMsg.includes('not init')) { // 入口之一，判断无初始化直接执行
            _initBluetooth(() => {
              _startConnectDevices()
            })
          }
  ...
  const _getCharacter = (services) => {
    services.forEach((service, index) => {
      if (service.uuid.includes(_serviceId)) {
      ...
      success: () => {
        ...
        wx.onBLECharacteristicValueChange((res) => {
          ...
          if (data.slice(0, 4) == 'aa10' || data.slice(0, 4) == 'aa12') {
            ...
            // 计算数据包长度
            _dataLen = Number.parseInt(data.slice(8, 12), 16) // 003e,16=62
            // 计算systemState
  ...
  const _analysisSystem = (systemState) => {
    // 16进制转换为10进制
    let decimalState = Number.parseInt(systemState, 16)
    // 10进制转换为2进制
    let binaryState = Number.parseInt(decimalState).toString(2)
  ...
  const _connectData = (data) => {
    ...
    if (_dataContent.length == _dataLen * 2) { // 接收完该长度的字节和校验CRC成功之后再发送ACK
      ...
      if (Number.parseInt(dataTransition.getCRC16(contentArr), 16) == Number.parseInt(_CRC16, 16)) {
        this.log('CRC16校验成功')
  ...
  const _analysisBLEContent = (content, reply) => {
    ...
    if (content.includes('020101')) {
    ...
    else if (content.includes('0300820100')) {
    ...
    else if (content.includes('0300810100')) {
      if (_sendCommandTimer) { // 每次都将清除等待任务
        clearTimeout(_sendCommandTimer)
        _sendCommandTimer = null
      }
    ...
    else if (content.includes('04008524')) {
      _writeBLECharacteristicValue(reply, () => {
        this.log('心跳包')
      })
    }
    else if (content.includes('020100')) {
      if (_sendCommandTimer) { // 每次都将清除等待任务
        clearTimeout(_sendCommandTimer)
        _sendCommandTimer = null
      }
    ...
    else if (content.includes('0300940100') || content.includes('0300940101')) {
      if (_sendCommandTimer) { // 每次都将清除等待任务
        clearTimeout(_sendCommandTimer)
        _sendCommandTimer = null
      }
    ...
    else if (content.includes('0400a8')) {
      // 查询车辆状态返回
      if (_sendCommandTimer) { // 每次都将清除等待任务
        clearTimeout(_sendCommandTimer)
        _sendCommandTimer = null
      ...
      if (!_hasReceive) {
        _hasReceive = true
        _writeBLECharacteristicValue(reply, () => {
          this.log(`查询头盔锁成功，开始回调ctrl_cb: ${content}`)
          this.end(() => {
            let result = null
            if (content.length >= 12) {
              let resLength = Number.parseInt(content.slice(6, 8), 16)
              if (resLength >= 2) {
                result = content.slice(8, 12)
              }
            }
  ...
  const _ctrl = (type) => {
    ...
    if (type == 'helmet') // 开头盔锁
      sendData = '03 00 14 01 01'
    if (type == 'status') // 获取车辆状态
      sendData = '04 00 28 01 00'
...
export default BluetoothOperate
